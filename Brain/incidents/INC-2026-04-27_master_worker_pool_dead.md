...
## Update 2026-04-28 — Bridge cron staging deshabilitado

**Razón:** staging no recibe emails reales (solo testing manual). El cron estaba reiniciando master_worker_staging cada hora innecesariamente (~24 restarts/día). Esto generaba ruido continuo y exponía DT-32 (sin reconnect logic).

**Acción:** crontab staging modificado para remover la línea `check_ingestion.sh`. Backup del crontab original guardado en `/home/ubuntu/crontab_backup_20260428.txt` para reactivar si necesario.

**Implicación:** staging queda sin auto-restart automático. Tests E2E deben asumir que master_worker_staging está controlado manualmente.

**Documentado en:** DT-39 (registrada como MITIGADA tras esta acción).

**Próximo paso:** Sprint fix de fondo (DT-32/33/34) inicia hoy 28/4 para resolver causa raíz del incidente.
