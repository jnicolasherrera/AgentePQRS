    ...
    while True:
        try:
            ...
            for email in new_emails:
                if not parece_pqrs(email['subject'], email['body'], email['sender']):
                    print(f"Ignorado (no es PQRS): {email['subject'][:60]}")
                    continue
                ...
                if resultado.tipo.value not in TIPOS_VALIDOS:
                    print(f"Descartado [{resultado.tipo.value}]: {email['subject'][:60]}")
                    continue
                        ...
                        if content:
                            ...
                            if storage_path:
                                ...
                                await conn.execute(
                                    ...
                                    db_id, TENANT_FLEXFINTECH, att['name'], storage_path, att['contentType'], att['size']
                                )
