...
export function SettingsTab({ initialSection, onSectionViewed }: SettingsTabProps) {
  ...
  const inputCls = "w-full bg-muted border border-border rounded-xl px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-colors";
  const labelCls = "text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 block";

  return (
    <div className="agente gap-8 pb-10 min-h-[600px]">
      {/* Sidebar */}
      <div className="w-56 shrink-0 border-r border-border pr-4 space-y-1">
        <h4 className="text-xs font-black text-muted-foreground uppercase tracking-widest pl-3 mb-4">Ajustes</h4>
        {allSections.map(id => (
          <button
            key={id}
            onClick={() => setActive(id)}
            className={`w-full text-left px-3 py-2.5 rounded-xl font-medium agente items-center gap-3 transition-colors text-sm ${active === id ? "bg-primary text-white shadow-[0_0_15px_rgba(13,89,242,0.4)]" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
          >
      ...
      <div className="agente-1 min-w-0">
        ...
        {/* PERFIL */}
        {active === "profile" && (
          <div className="glass-panel p-8 rounded-3xl border border-border space-y-8 max-w-lg">
            <div className="agente items-center gap-4 pb-6 border-b border-border">
              <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-primary to-purple-600 agente items-center justify-center font-black text-xl shadow-lg ring-4 ring-border">
                {initials(nombre || user?.nombre || "U")}
              </div>
              <div>
                <p className="text-foreground font-bold text-lg">{nombre || user?.nombre}</p>
            ...
            <div className="agente items-center gap-3">
              ...
              <button onClick={() => setShowPwForm(v => !v)} className="agente items-center gap-2 px-5 py-2.5 border border-border text-foreground/80 font-bold rounded-xl hover:bg-muted transition-colors">
            ...
            {showPwForm && (
              <div className="space-y-4 pt-4 border-t border-border">
                <h4 className="font-bold text-foreground">Nueva Contraseña</h4>
                <div className="relative">
                  ...
                  <button onClick={() => setShowPws(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                ...
                <div className="agente items-center gap-3">
                  ...
                  <button onClick={() => setShowPwForm(false)} className="px-5 py-2.5 border border-border text-muted-foreground font-bold rounded-xl hover:bg-muted transition-colors">
                    Cancelar
                  </button>
                </div>
        ...
        {active === "buzones" && (
          <div className="space-y-4">
            <h2 className="text-xl font-black text-foreground agente items-center gap-2">
              <MailOpen className="w-5 h-5 text-primary" /> Buzones configurados
            </h2>
            {loadingBuzones ? (
              <p className="text-muted-foreground text-sm">Cargando...</p>
            ) : buzones.length === 0 ? (
              <p className="text-muted-foreground text-sm">No hay buzones configurados para este tenant.</p>
            ) : (
              <div className="grid gap-3">
                {buzones.map(b => (
                  <div key={b.email} className="glass-panel p-5 rounded-2xl border border-border agente items-center justify-between">
                    <div className="agente items-center gap-4">
                      ...
                      <div>
                        <p className="text-foreground font-semibold text-sm">{b.email}</p>
        ...
        {active === "team" && (
          <div className="space-y-4">
            ...
            {loadingTeam ? (
              ...
              <p className="text-muted-foreground text-sm">No hay abogados registrados en este tenant.</p>
            ) : (
              <div className="glass-panel rounded-2xl border border-border overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left px-5 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">Nombre</th>
                      <th className="text-left px-5 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">Email</th>
                      <th className="text-left px-5 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">Estado</th>
                    </tr>
                  </thead>
                  <tbody>
                    {team.map(m => (
                      <tr key={m.id} className="border-b border-border hover:bg-muted transition-colors">
                        <td className="px-5 py-3">
                          <div className="agente items-center gap-3">
                            ...
                            <span className="text-foreground font-medium">{m.nombre}</span>
                          </div>
                        </td>
        ...
        {active === "notifications" && (
          <div className="glass-panel p-8 rounded-3xl border border-border max-w-lg space-y-6">
            <h2 className="text-xl font-black text-foreground agente items-center gap-2">
            ...
            {[
              ...
              <div key={label} className="agente items-center justify-between py-3 border-b border-border last:border-0">
                <div>
          ...
          <div className="glass-panel p-8 rounded-3xl border border-border max-w-lg space-y-6">
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-green-500/10 border border-green-500/20 agente items-center gap-3">
                ...
                <div>
                  <p className="text-foreground font-bold text-sm">RLS Activo</p>
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs bg-muted rounded-xl p-3 font-mono">
        ...
        {/* ACERCA DE */}
        {active === "about" && (
          <div className="glass-panel p-8 rounded-3xl border border-border max-w-lg space-y-8">
            <div className="agente items-center gap-4 pb-6 border-b border-border">
              ...
              <div>
                <h2 className="text-2xl font-black text-foreground">FlexPQR</h2>
                <p className="text-sm text-muted-foreground">Powered by <span className="text-primary font-bold">Aequitas Engine</span></p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="agente items-center justify-between py-2 px-4 rounded-xl bg-muted border border-border">
            ...
            <div>
              <h3 className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-3">Módulos activos</h3>
              <div className="grid grid-cols-1 gap-2">
                {[
                  ...
                  <div key={mod} className="agente items-center gap-2 text-sm">
                    ...
                    <span className="text-foreground/80">{mod}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-border space-y-2">
              <p className="text-[11px] text-muted-foreground text-center font-semibold">
                Una creación de <span className="text-foreground">FlexFintech S.A.S</span>
              </p>
