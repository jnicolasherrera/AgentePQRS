-- ═══════════════════════════════════════════════════════════════
-- Migración 00: Baseline del schema de PRODUCCIÓN (DDL puro)
-- ═══════════════════════════════════════════════════════════════
-- Origen: pg_dump --schema-only --no-owner --no-acl de pqrs_v2 en
--         prod 18.228.54.9, ejecutado 2026-04-23 16:00 UTC bajo
--         autorización explícita de Nico (ver
--         Brain/sprints/SPRINT_TUTELAS_S123_BASELINE_PROD_SCHEMA.md).
-- Archivo raw versionado en migrations/baseline/prod_schema_20260423_1600.sql
--         (SHA256 3d0bc89fd69b35819842f3e0db9eacf587cc4935cfce9bf031af339a17c14044).
-- Única diferencia con el raw: se removieron las directivas
-- \restrict/\unrestrict de pg_dump, que no aportan a la reconstrucción.
--
-- Cierra el drift repo↔prod documentado en
-- Brain/sprints/SPRINT_TUTELAS_S123_BLOQUEANTE_DRIFT_REPO.md. Las
-- SQLs históricas 01-05, 08 de la raíz del repo quedan como legacy
-- y NO se aplican en staging reconstruido.
--
-- Se aplica FIRST en el pipeline migrate.sh. Después la 14
-- (régimen sectorial) y al final 99_seed_staging.sql (fixture
-- sintético, solo staging).
-- ═══════════════════════════════════════════════════════════════
